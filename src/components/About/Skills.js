...
const Skills = () => {
  return (
    ...
    dark:text-light">
      <span className="font-semibold text-lg sm:text-3xl md:text-4xl text-accent dark:text-accentDark ">I'm confortable in ...</span>
      <ul className="flex flex-wrap mt-8 justify-center xs:justify-start">
        {SkillList.map((item, index) => {
          return (
            <li
              key={index}
              className="font-semibold capitalize inline-block text-base xs:text-lg  sm:text-xl md:text-2xl py-2 xs:py-3 sm:py-4 lg:py-5 px-4 xs:px-6 sm:px-8 lg:px-12 border-2 border-solid border-dark rounded mr-3 mb-3 xs:mr-4 xs:mb-4 md:mr-6 md:mb-6 dark:border-light hover:scale-105 transition-all ease duration-200 cursor-pointer
              dark:font-normal"
            >
              {item}
            </li>
